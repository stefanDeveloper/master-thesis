...
OpenSSH is one of the most used applications to acquire an SSH connection.
...
\begin{figure}[ht]
    ...
    \caption[OpenSSH architecture]{OpenSSH architecture}
    \label{fig:openssh-architecture}
\end{figure}

The architecture consists of three different components, namely \verb|ssh-connection|, \verb|ssh-userauth|, and \verb|ssh-transport|.

\begin{figure}[ht]
    \centering
    \includegraphics{figures/ssh-flow-diagram.pdf}
    \caption[OpenSSH flow diagram]{OpenSSH flow diagram}
...
For instance, we use the latest OpenSSH $8.8P1$.
The easiest step is to tweak the authentication to permit any connection in order to forward it to Cowrie.
Therefore, our server just returns true in the \verb|allowed_user| function to skip the rest of the authentication process.
In OpenSSH, communications are handled in so-called channels.
