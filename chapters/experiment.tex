...
In the RFC the maximum packet size is set to $32768$ bytes.% TODO: check rfc
%In the OpenSSH server configuration, any similar check could not be identified.
...
Any version higher (such $2.2$) than results in a \verb|bad version 2.2|.
Setting an older version $1.0$ returns the same as a higher version.
However, OpenSSH deviates from Cowire.
For instance, 
