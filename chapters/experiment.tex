...
Detecting honeypots before launching attacks helps to dismantle 
...
Our updated \verb|sshd| daemon is exposed to the Internet, and is accessible through \ipAddress{129.206.5.157:22}.
...
\begin{figure}
    \lstinputlisting[language=bash, caption={[Disguised Cowrie connection attempt with probed packets.]Disguised Cowrie connection attempt with probed key exchange initialization message and local string version}, label={lst:ssh-cowrie-fixed}]{listings/ssh-cowrie-fixed.txt}
\end{figure}
...
For instance, we have used \citet{vetterl2020} original $6.3P1$ sshd honeypot\footnote{\url{https://github.com/amv42/sshd-honeypot/commit/f58830161002baec9d3ed218e78ddb06b0d40a23}}, as well as the newly implemented version $8.8P1$ with Cowrie version $2.3.0$.
Both versions have been tested in heiCLOUD and locally in an encapsulated environment.
\autoref{lst:ssh-cowrie-fixed} shows the debug information of our connection with probed key exchange initialization message and local string version.
Both versions are capable of hiding, and forwarding any related packets to Cowrie.
Moreover, request do not result in a bad packet length, thus, behaving like an original sshd daemon.
Cowrie receives the connection and logs information respectively.
Currently, one downside is the connection loss that happens due to timeout restrictions.
This issue is minor, and could be fixed in the future.
