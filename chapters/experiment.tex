...
In addition, we have tested it with the ssh-rsa host key algorithm, and the response remains the same.
...
For instance, we use the default Cowrie implementation version $v.2.3.0$ of our T-Pot instance.
\autoref{lst:ssh-cowrie} outlines the connection attempt of Cowrie.
Unambiguously, Cowrie results in a bad packet length, and thus, deviates fundamentally from an OpenSSH response.
The underlying off-the-shelf library TwistedConch checks if a packet is within one megabyte.
Any packet that exceeds $1048576$ bytes causes a \verb|bad packet length *| exception that results in a connection aborting of the server.
...
In the RFC the maximum packet size is set to $32768$ bytes (256 KB).

Any adversary who modifies its SSH client to send our specific version string and key exchange initialization message could catch the exception and stop any further attacks.

Besides the bad packet length, another downside is that TwistedConch enforces a certain protocol version, namely either $1.99$ and $2.0$.
Any version higher or lower (such as $2.2$ and $1.0$) than results in a \verb|bad version 2.2|.
However, OpenSSH deviates from Cowrie.
For instance, sending a higher version string
%We found out that the version string causes this error.
%The library TwistedConch only accepts the version string 
%throws this exception. % TODO Debug Cowrie
