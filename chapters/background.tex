...
Mostly, low-interaction honeypots are chosen due to a significant reduce in risk, so that, adversaries are not be able to exploit honeypots to attack other systems.
...
In this way, more serious information about the attack can be gathered, including privilege assessment, toolkit capture, and command execution.
...
In production environments, it is not easy to find attacks for further data analysis.
...
To assess the value of honeypots we want to take a closer look at their advantages and disadvantages. \cite{Mokube2007,Kaur2014,Spitzner2003}
...
\begin{itemize}
    \item Independent of Workload: Honeypots only process traffic that is directed to them.
