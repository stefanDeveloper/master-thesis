...
\begin{figure}
    \lstinputlisting[language=python, caption={[TwistedConch packet length validation]TwistedConch packet length validation.}, label={lst:twistedconch}]{listings/twistedconch.txt}
\end{figure}

\begin{figure}
    \lstinputlisting[language=python, caption={[Cowrie version string validation]Cowrie version string validation. It tweaks the same results as OpenSSH.}, label={lst:cowrie-version-string}]{listings/cowrie.txt}
\end{figure}
...
Any connection obtain to OpenSSH will be forwarded to our honeypot by a NAT rule (\verb|iptables -t nat -A PREROUTING -p tcp --dport 22 -j REDIRECT --to-port 65222|).
