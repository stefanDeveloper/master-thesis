...
\begin{table}
    \caption{Overview of the cosine similarity of OpenSSH, Cowrie, and Twisted. (derived from \cite{vetterl2020})}
...
\section{Experiment 1: Reproduce Vetterl et al.'s findings} % TODO: Fill data
...
For the \verb|SSH2_MSG_KEXINIT| packet, the encryption algorithm blowfish-cbc and host key algorithm has been removed with version $7.6P1$, and thus, are outdated. % TODO: which host key algorithm
...
However, we found out that patches removed the blowfish-cbc for the SSH daemon, and thus, a reproduction is not feasible.
